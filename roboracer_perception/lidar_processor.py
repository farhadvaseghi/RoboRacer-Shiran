...
class LidarProcessor(Node):
    ...
    # laser frame is at x=+0.116 m, y=0, z=+0.088 m relative to base_link.
    _LASER_X_OFFSET = 0.116
    ...
    def _build_cone_array(self, header: Header, cones: list) -> ConeArray:
        ...
        The laser is at x=+0.116 m relative to base_link (same heading, no rotation),
        so base_link_x = laser_x + 0.116.  y is unchanged.
        ...
        for c in cones:
            cone = Cone()
            cone.x = c['cx'] + self._LASER_X_OFFSET  # laser → base_link: +0.116 m
            cone.y = c['cy']
